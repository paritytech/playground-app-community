...
import { READ_DEADLINE_MS, SIGN_DEADLINE_MS, withDeadline } from "../../utils/deadline.ts";

const MAX_WEIGHT = 18446744073709551615n;
// PAS has 10 decimals on Asset Hub (same base unit preflight.ts uses). The
// 2-PAS storage-deposit floor is the DotNS SDK reference default; spelling it
// as 2 × PAS avoids the 1e12 ("200 PAS") typo this constant used to carry.
const PAS = 10_000_000_000n;
const MIN_STORAGE_DEPOSIT = 2n * PAS; // 2 PAS
...
export async function isAccountMapped(signerAddress: string): Promise<boolean> {
    ...
    const test = await withDeadline(
        unsafeApi.apis.ReviveApi.call(
            signerAddress,
            ZERO_H160,
            0n,
            ...
            MAX_WEIGHT,
            Binary.fromHex("0x"),
            // BEST block, not PAPI's finalized default: this probes right
            // after map_account resolves at in-block — the finalized view
            // lags ~30-45s behind and would spin the propagation loop.
            { at: "best" },
        ),
        READ_DEADLINE_MS,
        "Probing the account mapping on Asset Hub",
    );
    const r = test as {
        result?: {
            value?: {
...
export async function ensureAccountMapped(
    ...
    // every 1s.
    //
    // WALL-CLOCK bounded, not attempt-bounded: each isAccountMapped is itself
    // deadlined at READ_DEADLINE_MS (45s), so an attempt counter of 60 was NOT
    // a "~60s ceiling" — on a wedged socket every probe burns its full deadline
    // and 60 attempts span ~45 minutes, a silent lockup behind "Mapping
    // account…". Cap the whole loop at ~90s of real time instead: a healthy
    // chain maps in a tick or two, and a dead one fails fast into the retryable
    // error below (the deploy reuses completed steps on re-run).
    const mappingDeadline = Date.now() + 90_000;
...
export function dryRunContractCall(
    ...
    return withDeadline(
        dryRunContractCallInner(contractAddress, callerAddress, encodedData, value, at),
        READ_DEADLINE_MS,
        "Reading contract state from Asset Hub",
    );
}

async function dryRunContractCallInner(
    contractAddress: string,
    callerAddress: string,
    encodedData: `0x${string}`,
    value: bigint = 0n,
    // BEST block by default, not PAPI's finalized default. The commit-reveal
    // flow dry-runs `register` ~12s after the commit resolved at in-block —
    // against the finalized head (lagging ~30-45s on Paseo) the commitment
    // is invisible and the whole retry window burns on CommitmentNotFound.
    // Pass "finalized" only where the point IS the resolver-visible state
    // (the post-deploy "is my site live yet" poll).
    at: "best" | "finalized" = "best",
): Promise<DryRunResult> {
    const { unsafeApi } = await getAssetHubClient();
...
export async function submitContractCall(
    ...
    // Use the dry-run estimate + 20% headroom when we have one — including a
    // legitimate 0n estimate, which `?:` would otherwise treat as "no estimate"
    // and bump to the floor. The floor only applies as a fallback.
    let storageDeposit =
        storageDepositEstimate !== undefined
    ...
    return withDeadline(
        submitAndWait(tx, signer, onStatus),
        SIGN_DEADLINE_MS,
        "Submitting the transaction to Asset Hub",
    );
}
